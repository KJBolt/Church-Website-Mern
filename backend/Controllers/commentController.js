...
export const addComment = async (req, res) => {
    ...
    try {
        const newComment = new Comment({...req.body, username:user.username, videoId:videoId, userId:req.user.id});
...
export const getReplies = async(req, res) => {
    try {
        const commentReplies = await Comment.find({parentId: req.params.commentId})
        if (commentReplies) {
